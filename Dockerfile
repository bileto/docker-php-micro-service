...
RUN apt-get install -y libicu48 libicu-dev php5-cli php5-dev php5-curl php5-pgsql php-pear libpcre3-dev \
  && apt-get clean purge autoremove -y
...
RUN wget https://github.com/phalcon/cphalcon/archive/phalcon-v1.3.4.tar.gz -O -| tar zx && cd cphalcon-phalcon-v1.3.4/build && ./install \
